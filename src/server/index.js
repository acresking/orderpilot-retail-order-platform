...
async function handleApi(req,res,pathname){
 ...
 // Public, unauthenticated: lets the login screen (both branch app and admin panel) show the
 // company's own logo/colors before anyone signs in, instead of flashing generic defaults first.
 if(method==='GET' && pathname==='/api/branding') return sendJson(res,200,{ companyName:db.meta?.companyName||'', companyLogo:db.meta?.companyLogo||'', brandColors:db.meta?.brandColors||null });
